...
# =========================
# Mode Selection with session_state
# =========================
...
# =========================
# Manual Entry Mode - FIXED BALANCE CALCULATION
# =========================
if mode == "manual":
    ...
    # ✅ FIXED: Correct cumulative balance calculation
    ...
    current_balance = starting_balance  # Start with user input
    forecast_data = []

    for m in range(1, months + 1):
        # Get income for this month (default or override)
        inc = income_changes.get(m, monthly_income)

        # Get expenses for this month
        var_exp = var_total  # Variable expenses are monthly recurring
    ...
    if uploaded_file is not None:
        try:
            ...
            user_instruction = st.text_area(
                "",
                value=f"This is my financial data. Automatically detect income, expenses, categories, time periods, and create a realistic {months}-month forecast with trends.",
                height=80
            )
...
if df_forecast is not None:
    ...
    # Balance trajectory
    ax1.plot(df_forecast['month'], df_forecast['cumulative_balance'],
             marker='o', linewidth=4, markersize=10, color='#2E86AB', label='Balance')
    ax1.fill_between(df_forecast['month'], df_forecast['cumulative_balance'], alpha=0.2, color='#2E86AB')
    ...
    with col1: st.metric("💰 Total Income", f"${total_income:,.0f}")
    with col2: st.metric("💸 Total Expenses", f"${total_expenses:,.0f}")
    with col3: st.metric("🏦 Final Balance", f"${final_balance:,.0f}")
    with col4: st.metric("📈 Avg Monthly Net", f"${avg_net:.0f}")
    ...
    st.download_button("📄 CSV", csv_data, f"yachay_forecast_{months}mo.csv", "text/csv")
